...
% and of the ensemble by unweighted average
...
error_tot = 0;
mean_eqmv_min = 0;
% Choice of the dataset to check performance
load('test.mat');
% load('train.mat');
N = length(X(:,1));
X = [X ones(N,1)];
...
figure(1);
plot(dengue_SP);
title('Original data from the original time series');
for fold=1:k,
    ...
    figure(fold+1);
    plot(S,'k');hold on;plot(Srn,'r');hold off;
    if fold == 2,
        title(sprintf('Desired output (black) X Prediction (red) for the %d-nd predictor',fold));
    elseif  fold == 3,
        title(sprintf('Desired output (black) X Prediction (red) for the %d-rd predictor',fold));
    else
        title(sprintf('Desired output (black) X Prediction (red) for the %d-th predictor',fold));
    end
end
